...
export default function AuthPage() {
    ...
    const handleAuthSubmit = async (e: React.SyntheticEvent) => {
        ...
        try {
            ...
            if (isLogin) {
                ...
                try {
                    const sceneResponse = await fetch(`/api/scene/load?username=${encodeURIComponent(username)}`);
                    if (sceneResponse.ok) {
                        const sceneData = await sceneResponse.json();
                        if (sceneData?.objects) setObjects(sceneData.objects);
                    }
                } catch (loadErr) {}
    ...
    const handleSaveLayout = async () => {
        if (!username) {
            setError('Cannot save layout: No authenticated user session found.');
            return;
        }

        setLoading('Saving layout...');
        try {
            const response = await fetch('/api/scene/save', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ username, objects })
            });

            if (!response.ok) throw new Error('Network response was not ok');
            
            setLoading('Layout saved securely!');
    ...
    return (
        <div className='flex h-screen w-screen overflow-hidden bg-white relative'>
            ...
            >
                {!isAuthenticated ? (
                    <div className='mx-auto w-full max-w-md'>
                        <div className='relative h-10 w-full mb-6'>
                            <h2 className={`absolute inset-0 text-3xl font-semibold tracking-tight transition-all duration-300 ease-in-out ${isLogin ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
                                Sign In
                            </h2>
                            <h2 className={`absolute inset-0 text-3xl font-semibold tracking-tight transition-all duration-300 ease-in-out ${!isLogin ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 pointer-events-none'}`}>
                                Create Account
                        ...
                        <form onSubmit={handleAuthSubmit} className='space-y-4'>
                            ...
                            {error && <p className='text-sm font-medium text-red-600 pt-1'>{error}</p>}
                            {loading && !error && <p className='text-sm font-medium text-[#D9A066] pt-1'>{loading}</p>}

                            <button type='submit' className='w-full rounded-xl bg-[#D9A066] hover:bg-[#c48e57] text-white py-3 px-4 font-medium text-sm transition-colors mt-4 shadow-sm'>
                                Submit
                            </button>
                            ...
                            <div className='mt-8 text-center'>
                                ...
                                >
                                    <span className={`absolute inset-0 flex justify-center items-center transition-all duration-300 ease-in-out ${isLogin ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform -translate-y-2 pointer-events-none'}`}>
                ...
                {isAuthenticated && (
                    <button
                        ...
                        title='Sign Out'
                    >
                        <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' strokeWidth={2} stroke='currentColor' className='w-5 h-5 group-hover:scale-105 transition-transform'>
            ...
            <div className={`bg-[#edd2a5] relative overflow-hidden h-full transition-all duration-700 ease-in-out z-10 ${isAuthenticated ? 'w-[calc(100vw-80px)]' : 'hidden lg:block lg:w-[55%]'}`}>
                    ...
                    <div className='bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl border border-[#e8c195]/20 text-[#4A3319] transform scale-100 transition-transform'>
                        ...
                        <div className='space-y-2.5 mb-6'>
                            <button 
                                ...
                                className={`w-full flex items-center justify-between p-3.5 rounded-xl border-2 transition-all duration-200 text-left group ${selectedType === 'cube' ? 'border-[#D9A066] bg-[#D9A066]/5 font-medium shadow-sm' : 'border-stone-100 bg-stone-50/50 hover:bg-stone-50 hover:border-stone-200'}`}
                            >
                                <span className={selectedType === 'cube' ? 'text-[#4A3319]' : 'text-stone-600'}>Cube</span>
                                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${selectedType === 'cube' ? 'border-[#D9A066] bg-[#D9A066]' : 'border-stone-300'}`}>
                            ...
                            <button 
                                ...
                                className={`w-full flex items-center justify-between p-3.5 rounded-xl border-2 transition-all duration-200 text-left group ${selectedType === 'sphere' ? 'border-[#D9A066] bg-[#D9A066]/5 font-medium shadow-sm' : 'border-stone-100 bg-stone-50/50 hover:bg-stone-50 hover:border-stone-200'}`}
                            >
                                <span className={selectedType === 'sphere' ? 'text-[#4A3319]' : 'text-stone-600'}>Sphere</span>
                                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${selectedType === 'sphere' ? 'border-[#D9A066] bg-[#D9A066]' : 'border-stone-300'}`}>
                            ...
                            <button 
                                ...
                                className={`w-full flex items-center justify-between p-3.5 rounded-xl border-2 transition-all duration-200 text-left group ${selectedType === 'custom' ? 'border-[#D9A066] bg-[#D9A066]/5 font-medium shadow-sm' : 'border-stone-100 bg-stone-50/50 hover:bg-stone-50 hover:border-stone-200'}`}
                            >
                                <span className={selectedType === 'custom' ? 'text-[#4A3319]' : 'text-stone-600'}>Torus Knot</span>
                                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${selectedType === 'custom' ? 'border-[#D9A066] bg-[#D9A066]' : 'border-stone-300'}`}>
                        ...
                        <div className='flex items-center justify-end space-x-3 pt-2 border-t border-stone-100'>
                            <button type='button' onClick={() => setIsModalOpen(false)} className='px-4 py-2.5 text-sm font-semibold text-stone-500 hover:text-stone-800 transition-colors'>
                                Cancel
                            </button>
                            <button type='button' onClick={handleAddObject} className='px-5 py-2.5 bg-[#D9A066] hover:bg-[#c48e57] text-white text-sm font-semibold rounded-xl transition-colors shadow-sm active:scale-95'>
                                Add to Scene
                            </button>
                        </div>
                    </div>
